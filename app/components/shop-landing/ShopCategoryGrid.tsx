...
async function getCategories(): Promise<{ categories: Category[]; failed: boolean }> {
  try {
...
export default async function ShopCategoryGrid() {
  const { categories, failed } = await getCategories();
    ...
    <section className="max-w-6xl mx-auto px-4 md:px-8 py-16 md:py-20">
      ...
      {failed ? (
        <p className="font-body text-sm text-charcoal-soft text-center">
          โหลดหมวดหมู่สินค้าไม่สำเร็จ กรุณาลองใหม่อีกครั้ง
        </p>
      ) : (
